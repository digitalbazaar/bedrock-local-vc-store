...
export class LocalVerifiableCredentialStore {
  ...
  async _queryByExample({credentialQuery, profileId}) {
    ...
    const _query = async ({example, trustedIssuer = []}) => {
      ...
      const requiredIssuer = query.map(q => q.issuer);
      const queryTypes = Array.isArray(queryType) ? queryType : [queryType];
      const requiredIssuers =
        Array.isArray(requiredIssuer) ? requiredIssuer : [requiredIssuer];

      const selector = {
        'meta.profileId': profileId,
        'content.type': {$in: queryTypes},
        'meta.issuer': {$in: requiredIssuers}
      };
      const {docs} = await this.db.find({
        selector,
        include_docs: true,
