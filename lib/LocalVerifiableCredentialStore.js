...
export class LocalVerifiableCredentialStore {
   ...
   */
  async find({displayable, parentId, profileId, type}) {
    let selector;
    if(type) {
      selector = {
        'meta.profileId': profileId,
        'content.type': {$eq: type},
        'meta.issuer': {$gte: null}
      };
    } else {
      selector = {
        'meta.profileId': profileId,
    ...
    const equals = [selector];
    selector = await edv.indexHelper.buildQuery({
      equals,
      hmac: edv.hmac
    });
    console.log(selector, '<><><><><><>selector:indexHelper.buildQuery');
    ...
    const indexAttributes = selector.equals.map(attribute => {
      return {
        name: {$eq: Object.keys(attribute)[0]},
        value: {$eq: Object.values(attribute)[0]}
      };
    });
    ...
    const docs = await Promise.all(response.docs.map(
      async doc => {
        return edv._decrypt({
          ...
          keyAgreementKey: edv.keyAgreementKey
        });
      }
    ));
    console.log(docs, '<><><><><><>decrypted docs');
    // filters out unwanted db data by just returning content & meta

    const returnValue =
      docs.map(({content, meta}) => ({content, meta}));
   ...
   *   credential.
   * @param {string} options.profileId - The ID of the profile.
   * @param {EdvClient} options.edv - An EDV Client instance.
   *
   * @returns {Promise<object>} Resolves with a confirmation of storage.
   */
  async insert({credential, meta = {}, profileId, edv}) {
    console.log('gets here:::::::insert');
    meta.issuer = this._getIssuer({credential});
    meta.profileId = profileId;
    const recipients = [{
      header: {
        kid: edv.keyAgreementKey.id,
    ...
    const encryptedDoc = await edv._encrypt({
      doc: {
        meta,
        content: credential
      },
      recipients,
      keyResolver: edv.keyResolver,
      hmac: edv.hmac
    });
    console.log(encryptedDoc, 'encryptedDoc:insert');
